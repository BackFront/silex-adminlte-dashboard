...
function core_load_modules($modulos, $app)
{
    foreach ($modulos as $moduleName) {
        core_autoload(BASEPATH . SAD_MODULE_PATH . DIRECTORY_SEPARATOR . $moduleName);

        // obtem todas as classes que devem ser instanciadas
        $autoInstanceFiles = core_get_file_by_dir(BASEPATH . SAD_MODULE_PATH . DIRECTORY_SEPARATOR . $moduleName . '/Controllers');
